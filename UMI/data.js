...
window.BSU_DATA = {
  ...
  "ablation": {
    ...
    "threshold": 0.65,
    "dissociation_note": "Switching off the word-selective units drops real-vs-fake-word accuracy from 0.98 to 0.54, past the 0.65 line, while a separate non-reading task only slips from 0.87 to 0.80 — so this is not a model that was simply damaged, which would lose both. But the average hides which half broke, and the honest version matters: the model still reads every real word correctly (1.00). What collapses is its ability to reject made-up words (0.075) — it starts calling almost everything a real word. That is a specific, localised deficit, which is the claim being made here, but it is the opposite half of the task from the one impaired in human dyslexia, where real-word reading is what suffers.",
    ...
    "reading": "Switching off the word-selective units drops the model's real-vs-fake-word accuracy to 0.54, past the line (0.65) below which a human would be called dyslexic, while switching off the same number of random neurons leaves it near 0.89, and a separate non-reading task is left mostly intact. So we didn't just make the model worse at everything. Read the 0.54 carefully though: real words are still read perfectly, and the entire drop comes from the model losing the ability to reject made-up words. The damage is specific, which is the point, but it is a different half of the task from the one human dyslexia impairs. Two caveats on the control: it is the mean of two random picks that landed at 0.96 and 0.81, so 'barely dents it' carries more spread than one number suggests. And the effect only appears once the model is large enough (see the note at left).\n\nThere is now a built-in Brain-Score test (the Yeatman2021-induced_dyslexia benchmark): it automatically finds the word-selective units, switches them off, and checks the random-neuron control before declaring 'dyslexia,' so simply damaging the model in a clumsy way never gets mistaken for the real, specific effect. The curves above did not come from it. They come from a separate 32B experiment, run before the benchmark existed. We re-ran that experiment from scratch in September 2026 to check it was not a one-off, and it held: the random-control line came back identical at every point, and the word-selective line landed at 0.51 against the original 0.54, still well past the threshold. The small gap is honest bookkeeping — one setting of the original run was never written down, so the re-run picks a slightly different set of units. Read the headline as 0.51 to 0.54 rather than exactly 0.54. The script and both sets of numbers now ship with Brain-Score so anyone with a big enough machine can repeat it. Separately, the built-in test on the 3B model it ships with moves reading from 0.90 to 0.79 with the random control at 0.95: the same specific deficit, too small to cross the line, and the test correctly declines to call it dyslexia. Both are shown; neither is dressed up as the other. We verified end-to-end that the control does its job: when an overly-broad switch-off hurt reading non-specifically, the test correctly refused to call it dyslexia."
  },
  "selection": {
