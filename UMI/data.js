...
window.BSU_DATA = {
  "meta": {
    "title": "Brain-Score · One Interface, Every Way a Model Can Be Brain-Like",
    "subtitle": "Brain-Score measures how closely an AI model resembles the brain. Until now, each kind of test (seeing, reading, listening, watching video, even acting in a game) needed its own custom setup. We rebuilt it so that once a model is wired in, the same single command tests it on every benchmark it's eligible for, with no new code per test. (Wiring in a brand-new kind of model is still real work; the win is that everything after that is uniform.) This page walks through what that makes possible.",
    ...
    "provenance": "How to trust the numbers: the behavior scores were re-run fresh this session and reproduced their earlier values exactly; the timing and game results come from real recorded brain data and real game playthroughs; the cortical maps are generated by our own tools. Anywhere a number is carried over from earlier work or is a stand-in for illustration, it's labelled as such."
  },
  "hero_rotation": [
  ...
  "movie_brain": {
    ...
    "subtitle": "A real person watched a 10-second movie clip (picture, sound, and dialogue) inside an fMRI scanner that recorded their brain. We then asked an AI model to predict that brain response from the very same clip. Top brain: what the person's brain actually did. Bottom brain: what the model predicted, for a clip it was never trained on. Both light up in step with the video as it plays (each map is a snapshot of the whole brain at that moment).",
    ...
    "tr_sec": 1.49,
    "cachebust": "14",
    "transcript": ["dialogue", "and", "sound", "play", "across", "the", "clip"],
    ...
    "caption": "Why this is a fair test: the model was trained on other episodes of the same show and never saw this clip, so the bottom brain is a genuine prediction, not a replay. The top brain is the real recorded scan. The source footage belongs to the research dataset (Algonauts / Courtois NeuroMod) and isn't reproduced here. (The clip is about 10 seconds: 7 brain snapshots, one roughly every 1.5 seconds.)"
  },
  "leaderboard": {
  ...
  "embodied_game": {
    ...
    "subtitle": "The same single hand-off that runs the brain tests also lets a model play a game: it sees a picture of the board, decides a move, the game updates, and round it goes, hundreds of times. It's a real Brain-Score test now, scored against a perfect player (the ceiling) and random moves (the floor).",
    "models": ["random moves", "Qwen-VL 3B (CoT)", "Qwen-VL 7B (CoT)", "Gemma-4 12B (CoT)", "DeepSeek-R1 (ASCII)", "perfect player"],
